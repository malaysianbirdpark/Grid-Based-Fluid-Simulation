Texture3D<float3> x_in : register(t0);
Texture3D<float3> b_in : register(t1);

RWTexture3D<float3> x_out : register(u0);
...
void main( uint3 DTid : SV_DispatchThreadID )
{
    ...
    const float3 up = x_in[uint3(DTid.x, min(DTid.y + 1, height - 1), DTid.z)];
