...
void main( uint3 DTid : SV_DispatchThreadID )
{
    ...
    const float3 up    = x_in[uint3(DTid.x, min(DTid.y + 1, height - 1), DTid.z)].xyz;
    const float3 down  = x_in[uint3(DTid.x, max(DTid.y - 1, 0), DTid.z)].xyz;
    ...
    div[DTid.xyz] = ((right.x - left.x) + (up.y - down.y) + (front.z - behind.z)) * 0.5f;
}