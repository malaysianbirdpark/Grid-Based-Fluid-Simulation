...
void main( uint3 DTid : SV_DispatchThreadID )
{
	...
	const min16float right = 
		obstacle[uint3(DTid.x + 1, DTid.y, DTid.z)].r <= 0.9f ? x_in[uint3(DTid.x + 1, DTid.y, DTid.z)].x : 0.0f;
	const min16float left = 
		obstacle[uint3(DTid.x - 1, DTid.y, DTid.z)].r <= 0.9f ? x_in[uint3(DTid.x - 1, DTid.y, DTid.z)].x : 0.0f;

	const min16float up = 
		obstacle[uint3(DTid.x, DTid.y + 1, DTid.z)].r <= 0.9f ? x_in[uint3(DTid.x, DTid.y + 1, DTid.z)].y : 0.0f;
	const min16float down = 
		obstacle[uint3(DTid.x, DTid.y - 1, DTid.z)].r <= 0.9f ? x_in[uint3(DTid.x, DTid.y - 1, DTid.z)].y : 0.0f;

	const min16float front = 
		obstacle[uint3(DTid.x, DTid.y, DTid.z + 1)].r <= 0.9f ? x_in[uint3(DTid.x, DTid.y, DTid.z + 1)].z : 0.0f;
