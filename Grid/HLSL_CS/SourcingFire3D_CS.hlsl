...
void main(uint3 DTid : SV_DispatchThreadID)
{
    ...
    //if (obstacle[DTid.xyz].r <= 0.9f) 
	{
		...
		//if (DTid.y > height - 5 && DTid.y <= height - 2)
		{
			...
			const min16float y = DTid.y - (min16float(height) * 0.5f);
			...
			if (x * x + y * y + z * z <= r * r)
			{
				...
				static const min16float t_rate = 100.0f;
				static const min16float t_target = 841.0f;
				const min16float delta_t = (1.0f - exp(-t_rate * dt)) * (t_target - quantity_in[DTid.xyz].g);
				const min16float delta_smoke = 0.0f;
				const min16float delta_f = 9.5f * dt;
				...
				//quantity[DTid.xyz] *= min16float4(dissipation, 0.999999f, dissipation, dissipation);
