    ...
    : Compute3DStage{ "3D-MCAdvection", "", 8, 8, 8 }
{
    ...
    pDevice->CreateUnorderedAccessView(_resource[1].Get(), nullptr, _uav[2].ReleaseAndGetAddressOf());
    pDevice->CreateShaderResourceView(_resource[1].Get(), nullptr, _srv[2].ReleaseAndGetAddressOf());

    // final
	desc.Format = DXGI_FORMAT_R16G16B16A16_FLOAT;
    pDevice->CreateTexture3D(&desc, nullptr, _resource[2].ReleaseAndGetAddressOf());
    pDevice->CreateUnorderedAccessView(_resource[2].Get(), nullptr, _uav[3].ReleaseAndGetAddressOf());
    pDevice->CreateShaderResourceView(_resource[2].Get(), nullptr, _srv[3].ReleaseAndGetAddressOf());
...
void MCAdvection3DStage::Run(ID3D11DeviceContext& context)
{
    ...
    context.CSSetShaderResources(0u, 3u, _srv[0].GetAddressOf());
...
ID3D11Resource* MCAdvection3DStage::Expose(int32_t attribute_id)
{
    if (attribute_id == _velocityOutID)
        return static_cast<ID3D11Resource*>(_resource[2].Get());
    return nullptr;
}
