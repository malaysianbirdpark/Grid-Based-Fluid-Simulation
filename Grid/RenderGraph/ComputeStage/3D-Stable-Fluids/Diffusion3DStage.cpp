...
Diffusion3DStage::Diffusion3DStage()
    : Compute3DStage{ "3D-Diffuson", "./CSO/Diffusion3D_CS.cso", 8, 8, 8 }
{
    _uav.resize(2);
    _srv.resize(2);
    _nullUav.resize(2);
...
void Diffusion3DStage::Run(ID3D11DeviceContext& context)
{
    SetBarrier(context);
    for (auto i{ 0 }; i != 20; ++i) {
...
void Diffusion3DStage::Consume(ID3D11Resource* resource, int32_t attribute_id)
{
...
ID3D11Resource* Diffusion3DStage::Expose(int32_t attribute_id)
{
